...
function sound(restaurant: boolean) {
  ...
  try {
    ...
    const play = (frequency: number, delay: number) => {
      const oscillator = context.createOscillator(); const gain = context.createGain();
      oscillator.frequency.value = frequency;
      gain.gain.setValueAtTime(0.0001, context.currentTime + delay);
      gain.gain.exponentialRampToValueAtTime(restaurant ? 0.13 : 0.07, context.currentTime + delay + 0.01);
      gain.gain.exponentialRampToValueAtTime(0.0001, context.currentTime + delay + 0.16);
      oscillator.connect(gain).connect(context.destination); oscillator.start(context.currentTime + delay); oscillator.stop(context.currentTime + delay + 0.18);
    };
    play(restaurant ? 880 : 620, 0); if (restaurant) play(1040, 0.2);
    window.setTimeout(() => void context.close(), 700);
  } catch { /* Browser sound restrictions are non-fatal. */ }
}
...
export function PushNotificationsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const sync = () => setPushEnabled(localStorageSafe() === "true");
  ...
  useEffect(() => {
    ...
    void subscribeForegroundMessages((payload) => {
      const data = payload.data ?? {}; const restaurant = data.type === "new_order";
      setToast({ title: payload.notification?.title || (restaurant ? "\u0648\u0635\u0644\u0643 \u0637\u0644\u0628 \u062c\u062f\u064a\u062f" : "\u062a\u062d\u062f\u064a\u062b \u0637\u0644\u0628\u0643"), body: payload.notification?.body ?? "", url: typeof data.url === "string" && data.url.startsWith("/") ? data.url : role === "restaurant" ? "/restaurant-dashboard/orders" : "/orders", restaurant });
      sound(restaurant); void refresh(); window.dispatchEvent(new CustomEvent("jibli:push", { detail: data }));
  ...
  const primeAudio = useCallback(() => { audioReady = true; sound(false); }, []);
