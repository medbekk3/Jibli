...
export function subscribeCustomerOrders(_: string, onData: (orders: OrderDocument[]) => void, onError: (error: Error) => void) { let active = true; const load = () => getCustomerOrders().then((orders) => { if (active) onData(orders); }).catch((error) => { if (active) onError(error instanceof Error ? error : new Error("تعذر تحميل الطلبات.")); }); void load(); const interval = window.setInterval(load, 10_000); return () => { active = false; window.clearInterval(interval); }; }
export function subscribeRestaurantOrders(_: string, onData: (orders: OrderDocument[]) => void, onError: (error: Error) => void) { let active = true; getRestaurantOrders().then((orders) => { if (active) onData(orders); }).catch((error) => { if (active) onError(error instanceof Error ? error : new Error("تعذر تحميل طلبات المطعم.")); }); return () => { active = false; }; }
