...
export async function PATCH(request: NextRequest) {
  try {
    const user = await requireNotificationUser(request); const documents = await getAllowedNotificationDocuments(user.uid, user.role); const batch = getAdminDb().batch();
    for (const document of documents) batch.set(getAdminDb().collection("notificationReads").doc(`${document.id}_${user.uid}`), { notificationId: document.id, userId: user.uid, readAt: FieldValue.serverTimestamp() }, { merge: true });
    await batch.commit();
