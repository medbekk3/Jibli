...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await adminDb.collection("products").doc(id).update({ ...input, updatedAt: FieldValue.serverTimestamp() });
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { const { restaurant } = await requireActiveRestaurantSession(); const { id } = await params; if (!await owned(id, restaurant.id)) return restaurantFailure("PRODUCT_NOT_FOUND", "الأكلة غير موجودة.", 404); await adminDb.collection("products").doc(id).delete(); return restaurantSuccess({ id }); }
