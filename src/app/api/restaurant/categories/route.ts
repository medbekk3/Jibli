...
export async function GET() {
  try {
    ...
    const [categories, products] = await Promise.all([getAdminDb().collection("categories").where("isActive", "==", true).get(), getAdminDb().collection("products").where("restaurantId", "==", restaurant.id).get()]);
