...
export default function RestaurantDashboardPage() {
  ...
  async function toggle() { if (updating) return; if (restaurant.isOpen && !window.confirm("هل تريد إغلاق المطعم أمام الطلبات الجديدة؟")) return; setUpdating(true); try { await setRestaurantOpenStatus(!restaurant.isOpen); await Promise.all([load(), account.refresh()]); } catch (caught) { setError(caught instanceof Error ? caught.message : "تعذر تحديث حالة المطعم."); } finally { setUpdating(false); } }
